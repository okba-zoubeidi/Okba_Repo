...
*** Test Cases ***
1. Login to Portal as DM and Delete Recurring ECC Report
    [Tags]    ECCReports
    Given I login to ${URL} with ${eccSupervisorUsername} and ${eccSupervisorPassword}
    And I switch to "eccreports" page
    And I select recurring report to delete   &{ReportValues}
    And I click on recurring delete button to delete report
...
Set Init Env
    ...
    \    Set To Dictionary    ${ReportValues}    ${key}    ${updated_val}
    \    ${updated_val}=    Replace String    ${ReportValues["${key}"]}    {reportName}    ${recurringReportName_ToDelete}
    \    Set To Dictionary    ${ReportValues}    ${key}    ${updated_val}