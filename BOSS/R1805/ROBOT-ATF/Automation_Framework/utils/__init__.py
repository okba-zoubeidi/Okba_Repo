...
class PPhoneInterface(object):
    ...
    def verify_pphone_active_audio_path(self, user, audiopath):
        ...
        if path == audiopath:
            ...
            raise Exception(
                "verify_pphone_active_audio_path: Audio path \"%s\" did not match \"%s\"" % (audiopath, path))
    ...
    def wait_for_call_appearance(self, user_callee, ca_type, ca_line):
        ...
        :raises ExceptionType: Raises robot exception if no match
        """
        ca_lines = {'right_line1': 'line0', 'right_line2': 'line1', 'right_line3': 'line2', 'right_line4': 'line3',
                    'left_line1': 'line4', 'left_line2': 'line5', 'left_line3': 'line6', 'left_line4': 'line7',}
    ...
    def pphone_si_verify_caller_number(self, user_callee, user_caller, ca, num_format):
        ...
        if re.match(pattern, pnum):
            ...
            raise Exception("pphone_si_verify_caller_number: user %s did not match re pattern %s with %s" % (
            user_caller.first_name, pattern, resp))
        return 0
    ...
    def pphone_verify_caller_number(self, user_callee, user_caller, ca, num_format):
        ...
        if re.match(pattern, caller_number):
            ...
            raise Exception(
                "pphone_verify_caller_number: Pattern %s did not match caller_number \"%s\" for number format %s " % (
                pattern, caller_number, num_format))
        return 0
