Testcase:  TC_289995_Verify that Account dropdown is disabled for global user in update TN Wizard.robot

*** Settings ***
Documentation   Verify that Account dropdown is disabled for global user in update TN Wizard.
...
*** Test Cases ***
Verify that Account dropdown is disabled for global user in update TN Wizard
    [Tags]    GLOBAL
    Given i login to ${URL} with ${bossUsername} and ${bossPassword}
    And I switch to "switch_account" page
    when I switch to account ${SCOCosmoAccount} with ${AccWithoutLogin} option
    When I switch to "phonenumber" page
    ${Updatephoneform}=    create dictionary  AccountActionId=disabled
