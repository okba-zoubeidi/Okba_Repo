...
*** Test Cases ***
Verify clicking on Save button it is moved to current povider name with first number and total number of phone number
    [Tags]  Regression  f1
    ...
    Then I verify the page "Transfer Numbers"
    Then I check "Save" is present
    Then I check number "${AOBTransferPhoneNumber['numberRange']}" on transfer number page
