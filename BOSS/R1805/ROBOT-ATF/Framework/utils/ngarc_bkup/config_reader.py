...
class ConfigReader:
    ...
    def get_gsuite_runset_path(self,conf):
        '''
        Author : mvilleda
        This will return the project runset path
        '''
        path=conf.FindNode("GSUITE_RUNSET_PATH").text
        if path is None:
            self.logger.error("script path info not found in config!")
        else:
            #path = path.text
            self.logger.debug("Script path is " + path)
        return path

    def get_gsuite_configs_path(self,conf):
        '''
        Author : mvilleda
        This will return the project configs path
        '''
        path=conf.FindNode("GSUITE_CONFIGS_PATH").text
        if path is None:
            self.logger.error("script path info not found in config!")
        else:
            # path = path.text
            self.logger.debug("get_gsuite_configs_path path is " + path)
        return path

    def get_gsuite_test_path(self, conf, branch, type=None):
        '''
        Author : mvilleda
        This will return the project test path
        '''
        if branch == "dev":
            if type is None:
                path=conf.FindNode("GSUITE_DEV_TEST_PATH")
            else:
                path=conf.FindNode("GSUITE_DEVPOM_TEST_PATH")
        else:
            if type is None:
                path=conf.FindNode("GSUITE_MAIN_TEST_PATH")
            else:
                path=conf.FindNode("GSUITE_MAINPOM_TEST_PATH")
    ...
    def get_gsuite_build_path(self, conf, branch):
        '''
        Author : mvilleda
        This will return the project build path
        '''
        if branch == "dev":
            path=conf.FindNode("GAPPS_DEV_BUILD_PATH")
        else:
            path=conf.FindNode("GAPPS_MAIN_BUILD_PATH")
    ...
    def get_gsuite_lib_path(self,conf):
        '''
        Author : mvilleda
        This will return the project lib path
        '''
        path=conf.FindNode("GAPPS_LIB_PATH").text
        if path is None:
            self.logger.error("script path info not found in config!")
        else:
            path = path.text
    ...
    def get_gsuite_backup_log_path(self,conf):
        '''
        Author : mvilleda
        This will return the project backup log path
        '''
        path=conf.FindNode("BACKUP_LOGPATH").text
        if path is None:
            self.logger.error("script path info not found in config!")
        else:
            # path = path.text
            self.logger.debug("get_gsuite_backup_log_path path is " + path)
        return path

    def get_gsuite_test_rerun_count(self,conf):
        '''
        Author : mvilleda
        This will return the project backup log path
        '''
        count=conf.FindNode("TEST_RERUN_COUNT").text
        if count is None:
            self.logger.error("test rerun count not found in config!")
        else:
            # count = count.text
            self.logger.debug("Rerun count is " + count)
        return count
