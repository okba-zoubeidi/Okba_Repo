###############################################################################
## Module: AssertElement
## File name: AssertElement.py
...
##  ---------   ---------      -----------------------
##  17-AUG-2014  VHA              Module created
###############################################################################
...
class AssertElement():
    ...
    def verify_text_in_dropdown(self, locator, valueToVerify):
        ''' Verifies the value identified by valueToVerify is present in list identified by textList
        
        '''
        try: 
            textList=self._browser.elements_finder(locator)
					
            count = 0
            for txt in textList:
                if txt.text == valueToVerify:
                    count = count +1
