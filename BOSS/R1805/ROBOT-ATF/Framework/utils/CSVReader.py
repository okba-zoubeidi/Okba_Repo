...
class CSVReader(object):
    ...
    def load_robot_automation_configs(self,auto_project = "TwW"):
        ...
        if auto_project == "GSuite":
            ...
            logger.error("Automation project %s was not found in %s\\%s" % (
            auto_project, OS_CONFIG_PATH, self._robot_runmodule_dir))

    def load_shoretel_users(self, csv_file_name):
        ...
        filename = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "automation-tww",
                                "Variables", csv_file_name)  ## in TwW used this location for csv file
